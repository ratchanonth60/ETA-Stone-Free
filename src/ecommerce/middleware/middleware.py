...
from ecommerce.core.tenants.utils import IsPublicSchema
...
class FlatpageFallbackMiddleware(IsPublicSchema, MiddlewareMixin):
    """
    Override for remove language url when call flatpage for get response
    """

    def process_response(self, request, response):
        if self.is_public_schema_request(request) or response.status_code != 404:
            return response  # No need to check for a flatpage for non-404 responses.
        try:
            url = request.path_info
            if len(settings.LANGUAGES) > 1 and f"/{request.LANGUAGE_CODE}/" in url:
                url = url.replace(f"/{request.LANGUAGE_CODE}/", "/", 1)

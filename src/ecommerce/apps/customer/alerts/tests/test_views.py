...
from ecommerce.apps.customer.models import ProductAlert
...
CustomerDispatcher = get_class("customer.utils", "CustomerDispatcher", "ecommerce.apps")
AlertsDispatcher = get_class(
    "customer.alerts.utils", "AlertsDispatcher", "ecommerce.apps"
)


class TestProductAlert(WebTestCase):
    def setUp(self):
        super().setUp()
...
class TestAUserWithAnActiveStockAlert(WebTestCase):
    def setUp(self):
        self.user = UserFactory()
    ...
    @mock.patch("ecommerce.apps.communication.utils.Dispatcher.notify_user")
    def test_site_notification_sent(self, mock_notify):
        self.stockrecord.num_in_stock = 10
        self.stockrecord.save()
        mock_notify.assert_called_once_with(
            self.user,
            f"{self.product.title} is back in stock",
            body=f'<a href="{self.product.get_absolute_url()}">{self.product.title}</a> is back in stock',
        )
    ...
    def test_product_title_truncated_in_alert_notification_subject(self, mock_notify):
        ...
        mock_notify.assert_called_once_with(
            self.user,
            f"{self.product.title[:200]} is back in stock",
            body=f'<a href="{self.product.get_absolute_url()}">{self.product.title}</a> is back in stock',
        )
...
class TestAlertMessageSending(TestCase):
    ...
    @mock.patch(
        "ecommerce.apps.communication.utils.Dispatcher.dispatch_direct_messages"
    )
    ...
    @mock.patch("ecommerce.apps.communication.utils.Dispatcher.dispatch_user_messages")

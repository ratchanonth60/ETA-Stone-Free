...
from ecommerce.apps.checkout.utils import CheckoutSessionData
from ecommerce.apps.order.models import BillingAddress, ShippingAddress
...
class CheckoutSessionMixin(object):
        ...
        for method_name in pre_conditions:
            if not hasattr(self, method_name):
                raise ImproperlyConfigured(
                    f"There is no method '{method_name}' to call as a pre-condition"
                )
    ...
    def get_pre_conditions(self, request):
        ...
        """
        return [] if self.pre_conditions is None else self.pre_conditions
    ...
    def check_skip_conditions(self, request):
        ...
        for method_name in skip_conditions:
            if not hasattr(self, method_name):
                raise ImproperlyConfigured(
                    f"There is no method '{method_name}' to call as a skip-condition"
                )
            getattr(self, method_name)(request)
    ...
    def get_skip_conditions(self, request):
        ...
        """
        return [] if self.skip_conditions is None else self.skip_conditions
    ...
    def skip_unless_payment_is_required(self, request):
        ...
        if shipping_method := self.get_shipping_method(
    ...
    def get_shipping_address(self, basket):
        ...
        if addr_data := self.checkout_session.new_shipping_address_fields():
            # Load address data into a blank shipping address model
            return ShippingAddress(**addr_data)
        if addr_id := self.checkout_session.shipping_user_address_id():
            try:
    ...
    def get_billing_address(self, shipping_address):
        ...
        if not self.checkout_session.is_billing_address_set():
            return None
        if self.checkout_session.is_billing_address_same_as_shipping() and shipping_address:
            address = BillingAddress()
            shipping_address.populate_alternative_model(address)
            return address

        if addr_data := self.checkout_session.new_billing_address_fields():
        ...
        if addr_id := self.checkout_session.billing_user_address_id():
    ...
    # def check_user_has_bankcard(self, request):
    #     user = request.user
    #     if not user.is_authenticated:
    #         return  # User is not logged in, skip checking

    #     bankcards = Bankcard.objects.filter(user=user)
    #     if not bankcards.exists():
    #         # No bankcards saved for this user
    #         raise exceptions.FailedPreCondition(
    #             url=reverse("payment:details"),
    #             message=_("Please add a bankcard before proceeding to checkout."),
    #         )

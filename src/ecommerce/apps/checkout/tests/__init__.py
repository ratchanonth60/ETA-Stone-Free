...
from ecommerce.apps.checkout.forms import GatewayForm
...
class CheckoutMixin(object):
    ...
    def enter_shipping_address(self):
        ...
        address_page = self.get(reverse("checkout:shipping-address"), user=self.user)
    ...
    def place_order(self):
        shipping_method = self.get(reverse("checkout:shipping-method"), user=self.user)
        payment_method = shipping_method.forms[1].submit().follow()
        preview = payment_method.click(linkid="view_preview")
    ...
    def reach_payment_details_page(self):
        ...
        return self.get(reverse("checkout:shipping-method"), user=self.user)

    def ready_to_place_an_order(self):
        shipping_method = self.reach_payment_details_page()
        payment_method = shipping_method.forms[0].submit().follow()
        return payment_method.click(linkid="view_preview")
...
class IndexViewPreConditionsMixin:
    ...
    @mock.patch(
        "ecommerce.apps.checkout.session.CheckoutSessionMixin.skip_unless_payment_is_required"
    )
    @mock.patch(
        "ecommerce.apps.checkout.session.CheckoutSessionMixin.skip_unless_basket_requires_shipping"
    )
...
class ShippingMethodViewSkipConditionsMixin:
    ...
    @mock.patch("ecommerce.apps.checkout.views.Repository")
...
class PaymentMethodViewSkipConditionsMixin:
    @mock.patch("ecommerce.apps.checkout.session.SurchargeApplicator.get_surcharges")
...
class PaymentDetailsViewMixin(
    ...
    @mock.patch("ecommerce.apps.checkout.views.PaymentDetailsView.handle_payment")
    ...
    @mock.patch("ecommerce.apps.checkout.views.logger")
    @mock.patch("ecommerce.apps.checkout.views.PaymentDetailsView.handle_payment")
    ...
    @mock.patch("ecommerce.apps.checkout.views.logger")
    @mock.patch(
        "ecommerce.apps.checkout.views.PaymentDetailsView.handle_order_placement"
    )
...
class PaymentDetailsPreviewViewPreConditionsMixin(PaymentDetailsViewPreConditionsMixin):
    ...
    @mock.patch(
        "ecommerce.apps.checkout.session.CheckoutSessionMixin.check_payment_data_is_captured"
    )

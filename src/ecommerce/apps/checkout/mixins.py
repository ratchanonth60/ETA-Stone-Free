...
from ecommerce.apps.order.utils import (
    OrderCreator,
    OrderDispatcher,
    OrderNumberGenerator,
)
from ecommerce.apps.payment.models import Transaction
...
class OrderPlacementMixin(CheckoutSessionMixin):
    ...
    def create_billing_address(
        ...
        """
        return self._extracted_from_create_billing_address(billing_address, user)

    # Rename this method here and in `create_shipping_address` and `create_billing_address`
    def save_billing_address(self, billing_address, user):
            ...
            with transaction.atomic():
                for source in self._payment_sources:
                    ...
                    Transaction.objects.create(
                        source=source,
                        txn_type=source.source_type.name,
                        amount=source.amount_allocated,
                        reference=source.reference,
                    )
        except Exception as e:
            # Handle the exception as appropriate for your application
            logger.error("Error saving payment sources: %s", e)
            # Depending on your error handling policy, you might want to re-raise the exception
            # or take other corrective actions here.
    ...
    def get_message_context(self, order):
        ...
        try:
            ...
            ctx["status_url"] = f"http://{site.domain}{path}"
        return ctx

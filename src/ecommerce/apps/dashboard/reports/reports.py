...
class ReportGenerator(object):
    ...
    def __init__(self, **kwargs):
        ...
        formatter_name = f"{kwargs.get('formatter', 'HTML')}_formatter"
    ...
    def filter_with_date_range(self, queryset):
        ...
        if self.start_date:
            ...
            filter_kwargs = {f"{self.date_range_field_name}__gte": start_datetime}
            queryset = queryset.filter(**filter_kwargs)

        # Before the end of the end date
        if self.end_date:
            end_datetime = utils.datetime_combine(self.end_date, time.max)
            filter_kwargs = {f"{self.date_range_field_name}__lte": end_datetime}
            queryset = queryset.filter(**filter_kwargs)
...
class ReportFormatter(object):
    def format_datetime(self, dt):
        return utils.format_datetime(dt, 'DATETIME_FORMAT') if dt else ''

    def format_date(self, d):
        return utils.format_datetime(d, 'DATE_FORMAT') if d else ''

...
from ecommerce.apps.order.models import Line, Order
from ecommerce.apps.partner.models import StockAlert
from ecommerce.apps.users.forms import AuthenticationForm
from ecommerce.apps.voucher.models import Voucher

from .utils import PopUpWindowMixin
...
class IndexView(TemplateView):
    ...
    :file:`eta/dashboard/index_nonstaff.html` template because Oscar's
    ...
    def get_template_names(self):
        if self.request.user.is_staff:
            return [
                "eta/dashboard/index.html",
            ]
        else:
            return ["eta/dashboard/index_nonstaff.html", "eta/dashboard/index.html"]
    ...
    def get_hourly_report(self, orders, hours=24, segments=10):
        ...
        *segments* defines the number of labelling segments used for the y-axis
        ...
        if max_value:
            ...
            y_range.extend(idx * y_axis_steps for idx in reversed(range(segments + 1)))
        else:
            for item in order_total_hourly:
                item["percentage"] = 0
...
class PopUpWindowCreateMixin(PopUpWindowCreateUpdateMixin):
    def popup_response(self, obj):
        ...
        return TemplateResponse(
            self.request,
            "eta/dashboard/widgets/popup_response.html",
            {
...
class LoginView(auth_views.LoginView):
    template_name = "eta/dashboard/login.html"

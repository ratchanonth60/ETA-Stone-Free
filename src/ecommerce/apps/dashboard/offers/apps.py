...
class OffersDashboardConfig(OscarDashboardConfig):
    ...
    name = "ecommerce.apps.dashboard.offers"
    ...
    def ready(self):
        DASHBOARD_OFFERS_VIEWS = "dashboard.offers.views"
        APPS_MODULE = "ecommerce.apps"

        self.list_view = get_class(DASHBOARD_OFFERS_VIEWS, "OfferListView", APPS_MODULE)
        self.metadata_view = get_class(
            DASHBOARD_OFFERS_VIEWS, "OfferMetaDataView", APPS_MODULE
        )
        self.condition_view = get_class(
            DASHBOARD_OFFERS_VIEWS, "OfferConditionView", APPS_MODULE
        )
        self.benefit_view = get_class(
            DASHBOARD_OFFERS_VIEWS, "OfferBenefitView", APPS_MODULE
        )
        self.restrictions_view = get_class(
            DASHBOARD_OFFERS_VIEWS, "OfferRestrictionsView", APPS_MODULE
        )
        self.delete_view = get_class(
            DASHBOARD_OFFERS_VIEWS, "OfferDeleteView", APPS_MODULE
        )
        self.detail_view = get_class(
            DASHBOARD_OFFERS_VIEWS, "OfferDetailView", APPS_MODULE
        )

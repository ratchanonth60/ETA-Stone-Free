...
from ecommerce.apps.dashboard.offers.forms import (
    BenefitForm,
    ConditionForm,
    MetaDataForm,
    OfferSearchForm,
...
from ecommerce.apps.dashboard.offers.reports import OrderDiscountCSVFormatter
from ecommerce.apps.offer.models import ConditionalOffer
from ecommerce.apps.order.models import OrderDiscount


class OfferListView(ListView):
    model = ConditionalOffer
    context_object_name = "offers"
    template_name = "eta/dashboard/offers/offer_list.html"
    ...
    def get_queryset(self):
        ...
        if name := self.form.cleaned_data["name"]:
...
class OfferWizardStepView(FormView):
    ...
    def _fetch_form_kwargs(self):
        session_data = self.request.session.setdefault(self.wizard_name, {})
        if json_data := session_data.get(self._key(self.step_name), None):
    ...
    def _fetch_object(self, step_name, request=None):
        ...
        if json_qs := session_data.get(self._key(step_name, is_object=True), None):
    ...
    def save_offer(self, offer):
        ...
        if benefit := self._fetch_object("benefit"):
            benefit.save()
            offer.benefit = benefit

        if condition := self._fetch_object("condition"):
            condition.save()
...
class OfferMetaDataView(OfferWizardStepView):
    ...
    template_name = "eta/dashboard/offers/metadata_form.html"
...
class OfferBenefitView(OfferWizardStepView):
    ...
    template_name = "eta/dashboard/offers/benefit_form.html"
...
class OfferConditionView(OfferWizardStepView):
    ...
    template_name = "eta/dashboard/offers/condition_form.html"
...
class OfferRestrictionsView(OfferWizardStepView):
    ...
    template_name = "eta/dashboard/offers/restrictions_form.html"
...
class OfferDeleteView(DeleteView):
    model = ConditionalOffer
    template_name = "eta/dashboard/offers/offer_delete.html"
...
class OfferDetailView(ListView):
    ...
    model = OrderDiscount
    template_name = "eta/dashboard/offers/offer_detail.html"
    ...
    def render_to_response(self, context):
        if self.request.GET.get("format") == "csv":
            ...
            return formatter.generate_response(
                context["order_discounts"], offer=self.offer
            )

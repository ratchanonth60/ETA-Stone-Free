...
from ecommerce.apps.dashboard.orders.forms import (
    OrderNoteForm,
    OrderSearchForm,
    OrderStatsForm,
    OrderStatusForm,
...
from ecommerce.apps.order.mixins import EventHandlerMixin
...
from ecommerce.apps.payment.models import SourceType, Transaction
...
class OrderStatsView(FormView):
    ...
    template_name = "eta/dashboard/orders/statistics.html"
...
class OrderListView(EventHandlerMixin, BulkEditMixin, ListView):
    ...
    template_name = "eta/dashboard/orders/order_list.html"
...
class OrderDetailView(EventHandlerMixin, DetailView):
    ...
    template_name = "eta/dashboard/orders/order_detail.html"
    ...
    def handle_line_action(self, request, order, action):
        ...
        line_quantities = []
        for line in lines:
            qty = request.POST.get(f"selected_line_qty_{line.id}")
            try:
                qty = int(qty)
            except ValueError:
                qty = None
    ...
    def get_order_note_form(self):
        ...
        if note_id := self.kwargs.get("note_id", None):
    ...
    def create_payment_event(self, request, order, lines, quantities):
        ...
        """
        if amount_str := request.POST.get("amount", None):
            try:
                amount = D(amount_str)
            except InvalidOperation:
                messages.error(request, _("Please choose a valid amount"))
...
class LineDetailView(DetailView):
    ...
    template_name = "eta/dashboard/orders/line_detail.html"
...
class ShippingAddressUpdateView(UpdateView):
    ...
    template_name = "eta/dashboard/orders/shippingaddress_form.html"
    ...
    def form_valid(self, form):
        ...
        if changes := get_change_summary(old_address, self.object):

...
from oscar.apps.search.features import is_solr_supported
...
for facet in settings.OSCAR_SEARCH_FACETS["queries"].values():
    field_name = f'{facet["field"]}_exact'
...
class SearchForm(FacetedSearchForm):
    ...
    def selected_multi_facets(self):
        ...
        for facet_kv in self.selected_facets:
            ...
            if field_name in VALID_FACET_QUERIES and value not in VALID_FACET_QUERIES[field_name]:
                continue
    ...
    def search(self):
        ...
        for field, values in self.selected_multi_facets.items():
            ...
            if field in VALID_FACET_QUERIES:
                ...
                sqs = sqs.narrow(f'{field}:({" OR ".join(values)})')
            else:
                # Field facet - clean and quote the values
                clean_values = [f'"{sqs.query.clean(val)}"' for val in values]
                sqs = sqs.narrow(f'{field}:({" OR ".join(clean_values)})')

        if self.is_valid() and "sort_by" in self.cleaned_data:
            if sort_field := self.SORT_BY_MAP.get(self.cleaned_data["sort_by"]):

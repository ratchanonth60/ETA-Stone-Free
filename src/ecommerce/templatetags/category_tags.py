...
class PassThrough(object):
    ...
    def __get__(self, obj, objtype):
        return self if obj is None else getattr(obj.category, self.name)
...
def get_annotated_list(depth=None, parent=None):    # noqa: C901 too complex
    ...
    for node in categories:
        ...
        if prev_depth is not None and node_depth < prev_depth:
            ...
            info["num_to_close"] = list(range(depth_difference))
            if tree_slugs := tree_slug.rsplit(
    ...
    if prev_depth is not None:
        # close last leaf
        info['num_to_close'] = list(range(prev_depth - start_depth))
